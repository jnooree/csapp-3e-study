...
int main(int argc, char **argv) {
  ...
  if (team_check) {
    ...
    if (!strcmp(team.teamname, "")) {
      printf("ERROR: Please provide the information about your team in "
             "mm.c.\n");
      exit(1);
    } else
      printf("Team Name:%s\n", team.teamname);
  ...
  if (errors == 0) {
    ...
    if (avg_mm_throughput > AVG_LIBC_THRUPUT) {
      ...
      p2 =
        ((double)(1.0 - UTIL_WEIGHT)) * (avg_mm_throughput / AVG_LIBC_THRUPUT);
    }
...
static double eval_mm_util(trace_t *trace, int tracenum, range_t **ranges) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case ALLOC: /* mm_alloc */
      ...
      /* Update statistics */
      max_total_size = (total_size > max_total_size) ? total_size
                                                     : max_total_size;
      break;
